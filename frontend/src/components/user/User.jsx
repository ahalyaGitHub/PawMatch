...
export default function User() {
    ...
    const fetchPets = async (query = '') => {
        try {
            ...
            setPets(availablePets);  // Set only available pets
        } catch (err) {
    ...
    const handleAdoptStatusChange = async (petId) => {
        ...
        if (token) {
            try {
                // Fetch the status before taking action
                    ...
                    {pets.length === 0 ? (
                        ...
                        pets.map((pet) => (
                            <div key={pet._id} className="bg-white shadow-lg overflow-hidden">
                                ...
                                <div className="p-4">
                                    <h5 className="text-xl font-bold text-gray-800">{pet.pet_id}</h5>
                                    ...
                                    <p className="text-gray-600"><span className="font-semibold">Vaccine:</span> {pet.vaccine} dose</p>

                                   
                                        <button
                                            onClick={() => handleAdoptStatusChange(pet._id)}
                                            className="bg-green-500 text-white py-2 w-full hover:bg-green-600 transition duration-300"
                                        >
                                            Adopt
                                        </button>
